...
// 包含 Adafruit_NeoPixel 库的头文件
#include <Adafruit_NeoPixel.h> 

// Replace with your network credentials
// WIFI_AP settings.
const char* AP_SSID = "dogdogdogdogdog";
...
static esp_err_t cmd_handler(httpd_req_t *req) {
    ...
    // val as servoID, cmdint as command.
        ...
        Serial.print(CurrentPWM[val]);
        Serial.print(" MID:");
        Serial.print(ServoMiddlePWM[val]);
        Serial.print(" offset:");
        ...
        if (debugMode) {
            ...
            Serial.print(val);
            Serial.print(" position:");
            Serial.println(ServoMiddlePWM[val]);
        } else {
    ...
    // 处理 buzzer 指令
    else if (!strcmp(variable, "buzzer")) {
        if (val == 1) {
            // 打开蜂鸣器的代码
            digitalWrite(BUZZER, LOW); // 假设低电平触发蜂鸣器
    ...
    // LED control
    else if (!strcmp(variable, "ledr") || !strcmp(variable, "ledg") || !strcmp(variable, "ledb")) {
        static int r = 0, g = 0, b = 0;
        if (!strcmp(variable, "ledr")) {
            r = val;
        } else if (!strcmp(variable, "ledg")) {
            g = val;
        } else if (!strcmp(variable, "ledb")) {
            b = val;
        }
        uint32_t color = matrix.Color(r, g, b);
        for (uint16_t i = 0; i < matrix.numPixels(); i++) {
            setSingleLED(i, color);
        }
    } 
    else {
